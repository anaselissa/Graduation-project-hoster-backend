...
function calcDistance(lat1, lng1, lat2, lng2) {
    return Math.round(
        Math.sqrt(
            ...
            Math.pow((lng2 - lng1) * 111, 2)
        ) * 10
    ) / 10;
}

// GET /api/map-locations/volunteers - للأهالي: يجيب المتطوعين القريبين
router.get('/volunteers', auth.authenticateToken, async (req, res) => {
    try {
        const { lat, lng, radius = 50 } = req.query;

        // جيب من volunteer_locations + users
...
// GET /api/map-locations/requests - للمتطوعين: يجيب طلبات الخدمة القريبة
